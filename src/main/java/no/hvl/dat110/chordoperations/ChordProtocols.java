	...
	public void fixFingerTable() {
		...
		try {
			...
			for(int i = 0; i < bits; i++) {
				BigInteger s = chordnode.findSuccessor(chordnode.getNodeID().add(BigInteger.valueOf(2).pow(i))).getNodeID().mod(addressSize);
				NodeInterface successor = chordnode.findSuccessor(s);
				if (successor != null) {
					ftable.add(successor);
				}
			}
