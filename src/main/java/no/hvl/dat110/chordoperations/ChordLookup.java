...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		try {
			...
			if (Util.checkInterval(key,node.getNodeID().add(BigInteger.ONE),successor.getNodeID())) {
				...
				if (predecessor != null && !predecessor.getNodeID().equals(node.getNodeID())) {
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		for (int i = fTable.size() - 1; i>=0; i--) {
			NodeInterface fnode = fTable.get(i);
			if (Util.checkInterval(fnode.getNodeID(),node.getNodeID().add(BigInteger.ONE),ID.subtract(BigInteger.ONE))) {
				return fnode;
			}
		}
		return null;
